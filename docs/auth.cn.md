...
| 字段名         | 类型   | 描述                               |
| -------------- | ------ | ---------------------------------- |
| `PK`           | String | Access Key ID, `base64url(UUIDv4)` |
| `SK`           | String | 固定值 `ACCESS_CREDENTIAL`         |
| `public_key`   | Binary | Public Key of the Ed25519 pair     |
| `tenant_id`    | String | Tenant ID                          |
| `created_at`   | Number | 创建时间(Unix时间戳, 毫秒)         |
| `last_used_at` | Number | 最后使用时间(Unix时间戳，毫秒)     |
| `is_active`    | bool   | 是否处于激活状态，默认是`false`    |
