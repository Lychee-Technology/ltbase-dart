...
void main(List<String> arguments) async {
  final parser = ArgParser()
    ...
    ..addOption('access-key-id',
        help: 'Access Key ID (format: AK_xxx)',
        mandatory: true)
    ..addOption('access-secret',
        help: 'Access Secret (format: SK_xxx)',
        mandatory: true)
    ..addFlag('verbose',
        abbr: 'v',
...
ArgParser _createDeeppingParser() {
  return ArgParser()
    ..addOption('echo', help: 'Echo string to send');
}

ArgParser _createCreateNoteParser() {
  return ArgParser()
    ..addOption('user-id',
        help: 'User ID of the creator',
        mandatory: true)
    ..addOption('type',
        help: 'Note type: text|audio|image',
        mandatory: true)
    ..addOption('data',
        help: 'Note data (text or URL)')
    ..addOption('file',
        help: 'Read data from file (alternative to --data)');
}
...
ArgParser _createListNotesParser() {
  return ArgParser()
    ...
    ..addOption('items-per-page',
        help: 'Items per page',
        defaultsTo: '20')
    ..addOption('schema-name',
        help: 'Filter by schema name (exact match)')
    ..addOption('summary',
        help: 'Filter by summary (contains)');
}

ArgParser _createUpdateNoteParser() {
  return ArgParser()
    ..addOption('summary',
        help: 'New summary text',
        mandatory: true);
}
...
Future<void> _executeCommand(CommandHandler handler, ArgResults command) async {
  switch (command.name) {
    ...
    case 'create-note':
      ...
      if (!['text', 'audio', 'image'].contains(type)) {
    ...
    case 'get-note':
      if (command.rest.isEmpty) {
        ...
        print('Usage: ltbase get-note <NOTE_ID>');
        exit(1);
      }
      await handler.getNote(command.rest.first);
      break;
    ...
    case 'update-note':
      if (command.rest.isEmpty) {
        print('Error: Note ID is required');
        print('Usage: ltbase update-note <NOTE_ID> --summary <SUMMARY>');
        exit(1);
      }
...
void _printUsage(ArgParser parser) {
  ...
  print('\nExamples:');
  print('  # Health check');
  print('  ltbase --access-key-id AK_xxx --access-secret SK_xxx deepping --echo "hello"');
  print('');
  print('  # Create text note');
  print('  ltbase --access-key-id AK_xxx --access-secret SK_xxx \\');
  print('    create-note --user-id user123 --type text --data "My note"');
  print('');
  print('  # List notes');
  print('  ltbase --access-key-id AK_xxx --access-secret SK_xxx \\');
